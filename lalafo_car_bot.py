...
CHAT_ID = os.getenv("CHAT_ID")          # один получатель
...
def send_telegram(text, photo_url=None):
    """Отправляет сообщение одному получателю (CHAT_ID)"""
    if not CHAT_ID:
        print("Ошибка: не задан CHAT_ID")
        return
    if not BOT_TOKEN:
...
def main():
    if not BOT_TOKEN:
        ...
        return
    if not CHAT_ID:
        print("Ошибка: укажите CHAT_ID в .env")
        return

    print(f"Бот запущен. Получатель: {CHAT_ID}")
